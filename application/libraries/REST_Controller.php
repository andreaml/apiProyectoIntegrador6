...
abstract class REST_Controller extends CI_Controller
{
	...
	*/
	...
	public function __construct()
	{
		...
		{
			$this->response(array('status' => false, 'error' => 'Solo se aceptan solicitudes AJAX.'), 505);
		}
	}
	...
	public function _remap($object_called, $arguments)
	{
		...
		{
			$this->response(array('status' => false, 'error' => 'Protocolo no soportado'), 403);
		}
		...
		if (config_item('rest_enable_keys') AND $use_key AND $this->_allow === FALSE)
		{
			...
			{
				$this->response(array('status' => false, 'error' => 'Esta API key no tiene acceso al controlador solicitado.'), 401);
			}
			...
			$this->response(array('status' => false, 'error' => 'API key inválida.'), 403);
		}

		// Sure it exists, but can they do anything with it?
		if ( ! method_exists($this, $controller_method))
		{
			$this->response(array('status' => false, 'error' => 'Método no conocido.'), 404);
		}
		...
		if (config_item('rest_enable_keys') AND !empty($this->rest->key))
		{
			...
			{
				$this->response(array('status' => false, 'error' => 'Esta API key ha alcanzado el límite horario para este método.'), 401);
			}
			...
			$authorized OR $this->response(array('status' => false, 'error' => 'Esta API key no tiene suficientes permisos.'), 401);
		}
	...
	protected function _check_whitelist_auth()
	{
		...
		{
			$this->response(array('status' => false, 'error' => 'No autorizado'), 401);
		}
	}
